...
const pincodedetiles = (props) => {
    return (
        <ul className="searchbar_result">
        {pincodesdata.filter((val) => {
            if (props == "") {
                return
            } else if (val.Name.toLowerCase().includes(props.searchedval.toLowerCase())) {
                return val
            }
        }).map((postcode) => {
